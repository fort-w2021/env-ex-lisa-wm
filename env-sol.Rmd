...
anywhere <- function(name, env = parent.frame()) {
  ...
  if (exists(name, env, inherits = TRUE)) {
    
    parents <- env_parents(env, last = empty_env())
    
    is_drin <- logical(length = length(parents))
    
    for (i in parents) {
      if (exists(name, parents$i)) {is_drin <- TRUE}
    }
  ...
  else anywhere(name, parent.env(env))
